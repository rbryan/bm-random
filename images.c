...
matrix *
newMatrix (int width, int height)
{
  int i;
...
void
saveImage (image * img, const char *filename)
{
  Imlib_Image out;
  int i, j;
...
gradient (matrix * mat)
{
  ...
  for (i = 1; i < width - 1; i++)
      ...
      for (j = 1; j < height - 1; j++)
	{
	  ...
	  grad->vals[i][j] =
	    (abs (pix[current_dir] - ((int) mat->vals[i][j])));
...
void
threshold (matrix * mat, matrix * dest, uint8_t min, uint8_t max)
{
  int i, j;
...
void
fill_matrix (matrix * mat, uint8_t value)
{
  int i, j;
