...
extern void floodfill_work (matrix * mat, particle * part, int x, int y,
			    int *id);
  ...
  for (i = 0; i < width; i++)
      ...
      for (j = 0; j < height; j++)
	{
	  ...
	  if (part->size > MAX_PART_SIZE)
	    {			//This if statement removes any particles that are too large or too small.
	      freeParticle (part);
	      continue;
	    }
...
void
drawline (matrix * mat, int val, int x1, int y1, int x2, int y2)
{
  int i, j, m;
  int xlarge, xsmall;
  if (x2 > x1)
...
void
drawparts (matrix * mat, partlist * parts, uint8_t val)
{
  int k, pixel;
  particle *current = 0;
  pt *cur_pixel = 0;
  ...
  for (k = 0; k < parts->size; k++, current = current->nextParticle)
    {
      ...
      for (pixel = 0; pixel < current->size;
	   pixel++, cur_pixel = cur_pixel->nextPt)
	{
      ...
      drawline (mat, 200, current->x, current->y, current->nextFrame->x,
		current->nextFrame->y);
      cur_pixel = current->nextFrame->spt;
      for (pixel = 0; pixel < current->nextFrame->size;
	   pixel++, cur_pixel = cur_pixel->nextPt)
	{
...
particle *
floodfill (matrix * mat, int id, int x, int y)
{
...
void
floodfill_work (matrix * mat, particle * part, int x, int y, int *id)
{
